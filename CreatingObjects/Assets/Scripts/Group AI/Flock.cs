...
public class Flock : MonoBehaviour
{
   ...
   [Range (0,10)]public float maximum = 10;
    ...
    Vector3 Alignment(List<Flock> neighbours)
    {
    ...
    Vector3 Cohesion(List<Flock> neighbours, Vector3 position)
    {
        if (neighbours.Count == 0)
        {
            return Vector3.zero;
    ...
    Vector3 Seperation(List<Flock> neighbours, Vector3 position)
    {
        if (neighbours.Count == 0)
        {
            return Vector3.zero;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            neighbours.Add(other.GetComponent<Flock>());
        }
    }

    private void OnTriggerExit(Collider other)
    {
        if (other.GetComponent<Flock>() && neighbours.Contains(other.GetComponent<Flock>()))
        {
            neighbours.Remove(other.GetComponent<Flock>());
        }
    }
}
